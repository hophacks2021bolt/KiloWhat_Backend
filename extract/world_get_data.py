...
owid_cols = ["iso_code", "year", "energy_per_capita", "renewables_share_energy", "biofuel_share_energy", "coal_share_energy",
    ...
    for x in range(len(df["iso_code"])):
        if (df["year"][x] >= 1990):
            if df["iso_code"][x] not in dict:
                dict[df["iso_code"][x]] = {str(df["year"][x]): {"avgEnergyUsage": str(df["energy_per_capita"][x])}}
            else:
                dict[df["iso_code"][x]] [str(df["year"][x])] = {"avgEnergyUsage": str(df["energy_per_capita"][x])}
    return dict
...
def renewable_energy_share():
    ...
    for x in range(len(df["iso_code"])):
        if (df["year"][x] >= 1990):
            if df["iso_code"][x] not in dict:
                dict[df["iso_code"][x]] = {str(df["year"][x]): {"renewableEnergyShare": str(df["renewables_share_energy"][x])}}
            else:
                dict[df["iso_code"][x]] [str(df["year"][x])] = {"renewableEnergyShare": str(df["renewables_share_energy"][x])}
    return dict

#Most common energy for each iso_code, each year
        ...
        if (df["year"][x] >= 1990):
            for i in sources:
                ...
                if df[col][x] > max:
                    ...
                    highest_share = i
            if df["iso_code"][x] not in dict:
                dict[df["iso_code"][x]] = {str(df["year"][x]): {"highestShare": highest_share}}
            else:
                dict[df["iso_code"][x]] [str(df["year"][x])] = {"highestShare": highest_share}
    return dict
